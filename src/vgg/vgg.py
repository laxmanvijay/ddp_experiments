...
def save_model(model, optimizer, epoch, accuracy, save_dir, filename):
    ...
    checkpoint = {
        ...
        'model_name': 'vgg16',
        'args': {
            'num_classes': model.module.classifier[6].out_features,  # VGG uses classifier[6] for final layer
        }
    }
...
def load_model(args, model, optimizer, load_path):
    ...
    if 'args' in checkpoint:
        ...
        current_num_classes = model.module.classifier[6].out_features if hasattr(model, 'module') else model.classifier[6].out_features
...
def train(args):
    ...
    model = models.vgg16(num_classes=args.num_classes).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Save checkpoint every few epochs for resuming
        if rank == 0 and (epoch + 1) % 5 == 0:  # Save every 5 epochs
            save_model(model, optimizer, epoch, best_accuracy, args.save_dir, f"checkpoint_epoch_{epoch}.pth")
            print(f"Checkpoint saved at epoch {epoch}")

    # Save final model after training (only rank 0)
    if rank == 0:
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./saved_vgg_model', 
