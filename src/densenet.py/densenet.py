...
def save_model(model, optimizer, epoch, accuracy, save_dir, filename):
    ...
    checkpoint = {
        ...
        'args': {
            'num_classes': model.module.classifier.out_features,  # DenseNet uses 'classifier' not 'fc'
        }
    }
...
def load_model(args, model, optimizer, load_path):
    ...
    if 'args' in checkpoint:
        ...
        current_num_classes = model.module.classifier.out_features if hasattr(model, 'module') else model.classifier.out_features
...
def train(args):
    ...
    model = models.densenet121(num_classes=args.num_classes).to(device)
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./saved_densenet_model', 
